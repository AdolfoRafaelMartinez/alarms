   ...
   '$timeout',
	...
	function($timeout) {
        ...
        var addHandlers = function(ap) {
            ...
            ap.on('pressmove', function(evt) {
                ...
                for (i=0; i<l; i++) {
                    ...
                    names[i].graphics.clear().setStrokeStyle(1).beginStroke('#ccc')
            ...
            ap.on('rollover', function(evt) {
                this.children[0].graphics.clear().beginFill('#cdf').drawCircle(0, 0, radius);
                update = true;
            });

            ap.on('rollout', function(evt) {
                this.children[0].graphics.clear().beginFill('DeepSkyBlue').drawCircle(0, 0, radius);
                update = true;
            });
        };
            ...
            for (i=0; i<layers_length; i++) {
                ...
                _.each(layers[i].children, function(ap2) {
                    if (ap != ap2) {
                        ...
                        m.graphics.setStrokeStyle(1).beginStroke('#ccc').moveTo(ap.x, ap.y).lineTo(ap2.x, ap2.y);
                        m.name = getNewName();
        ...
        this.addAP = function(evt, signal_radius) {
            ...
            var text = new createjs.Text('AP ' + ap_index++, "14px Arial", "#000000");
            text.x = -15;
            text.y = -10;
            text.textBaseline = "alphabetic";
                    ...
                    if (child.puddle_type == 'ap') {
                        for (var i=0; i<child.children.length; i++) {
                            if (child.children[i].puddleShape == 'signal') {
                                child.children[i].graphics.clear().beginFill('DeepSkyBlue').drawCircle(0, 0, radius);
                            }
                        }
                    }
                });
